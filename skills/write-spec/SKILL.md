...
Do not create Feature Issues from this skill. Once the Spec is accepted, cut its scope into Feature Issues. Route to `write-issues` when issue tracking would clarify execution, and to `implement` only when the user tells the agent to build a specific piece. An accepted Spec is not that go-ahead; writing or accepting it never authorizes building on its own.
