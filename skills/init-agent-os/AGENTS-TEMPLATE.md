...
- Use Linear as the issue tracker (team `[TEAM]`, project `[project]`); an issue id like `[TEAM]-123` resolves there. It owns Decision Maps, Decision Issues, Specs, Feature Issues, and Sub-Issues. Feature Issues are vertical slices of user-recognizable behavior that nest shallowly, not technical tasks or horizontal layers. (Use the project's own tracker instead when it has one.)
